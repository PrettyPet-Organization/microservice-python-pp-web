...
class HackathonParticipationRequest(models.Model):
    ...
    profile = models.ForeignKey(
        to=Profile, on_delete=models.CASCADE, verbose_name=_("profile")
    )
    hackathon = models.ForeignKey(
        to=Hackathon, null=True, on_delete=models.SET_NULL, verbose_name=_("hackathon")
    )
    cover_letter = models.TextField(
        max_length=400, blank=True, null=True, verbose_name=_("cover letter")
    )
    resume_url = models.CharField(
        blank=True, null=True, max_length=100, verbose_name=_("resume url")
    )
